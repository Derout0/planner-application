...
type Props<T extends ElementType> = RippleProps<T> &
	Omit<ComponentPropsWithoutRef<T>, keyof RippleProps<T>>

export const Ripple = forwardRef(
	<T extends ElementType = 'div'>(props: Props<T>, ref: Ref<Element>) => {
		const {
			className,
			children,
			as: Component = 'div',
		...
		const start = (event: SyntheticEvent) => {
			...
			if (center) {
				...
				const sizeX =
					Math.max(
						Math.abs(
							(elementRef.current ? elementRef.current.clientWidth : 0) -
								rippleX
						),
						rippleX
					) *
				...
				const sizeY =
					Math.max(
						Math.abs(
							(elementRef.current ? elementRef.current.clientHeight : 0) -
								rippleY
						),
						rippleY
					) *
		...
		return (
			<Component
				ref={mergedRef}
				className={clsx(cls.Ripple, {}, [className])}
